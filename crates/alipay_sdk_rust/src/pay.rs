...
use crate::biz::{
    ...
    TradeRefundBiz, TradeWapPayBiz, SystemOAuthTokenBiz,
};
...
use crate::response::{
    ...
    TradePrecreateResponse, TradeQueryResponse, TradeRefundResponse, SystemOauthTokenResponse,
};
...
pub trait Payer {
    ...
    fn system_oauth_token(&self, biz_content: &SystemOAuthTokenBiz) -> Result<SystemOauthTokenResponse>;
}
...
pub struct PayClient {
    ...
    notify_url: String, //`json:"notify_url"`           //否
}
...
impl Payer for PayClient {
    ...
    fn system_oauth_token(&self, biz_content: &SystemOAuthTokenBiz) -> Result<SystemOauthTokenResponse> {
        let body = self.do_alipay(biz_content)?;
        let res: SystemOauthTokenResponse = serde_json::from_slice(&body)?;
        if res.response.code.is_some() {
            log::debug!("{}", serde_json::to_string(&res)?);
            return Err(Error::new(
                ErrorKind::Other,
                format!(
                    "system_oauth_token failed: {} code:{}",
                    res.response.msg.unwrap_or(String::from("")) + "-" + res.response.sub_msg.unwrap_or(String::from("")).as_str(),
                    res.response.code.unwrap_or(String::from("")) + "-" + res.response.sub_code.unwrap_or(String::from("")).as_str()
                ),
            ));
        }
        Ok(res)
...
pub struct PayClientBuilder<'a> {
    ...
    notify_url: Option<&'a str>, // `json:"notify_url"`
}
        ...
        match biz_content.method().as_str() {
            ...
            _ => {
                ...
                request
                    ...
                    .set_param(biz_content)
                    .set_method(biz_content.method().as_str());
                let mut res = self.execute(&mut request)?;
                
                let res_str = std::str::from_utf8(&res);
                match res_str {
                    Ok(x) => {
                        let key = biz::get_response_key(biz_content);
                        let err_key = biz::get_error_response_key();
                        let new_res = x.replace(err_key.as_str(), key.as_str());
                        log::debug!("{}", new_res);
                        res = new_res.as_bytes().to_vec();
...
impl<'a> PayClientBuilder<'a> {
    ...
    /// 设置后后台回调地址 notify_url 自动跳转回商户页面
