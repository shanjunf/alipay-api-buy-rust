...
use crate::{
    ...
    pay::{PayClient, Payer},
    sign::{builder, Signer},
    util::{get_biz_content_str, get_now_beijing_time_str, get_param_map},
};
use std::{
...
pub trait Requester {
    ...
    fn set_param(&mut self, b: &impl BizContenter) -> &mut Self;
...
pub struct Request {
    ...
    biz_content: Option<String>, //`json:"biz_content"`              //是		请求参数的集合，最大长度不限，除公共参数外所有请求参数都必须放在这个参数中传递，具体参照各产品快速接入文档
    #[serde(skip_serializing_if="Option::is_none")]
    param: Option<String>                                   //不是biz_content内的参数
}
...
impl Request {
    ...
    fn get_sorted_sign_source(&self) -> Result<String> {
        ...
        if let Some(p) = &self.borrow().param {
            m = serde_json::from_str(p.as_str())?;
            m.iter().for_each(|(k, v)| {
                if !v.is_empty() {
                    let query = format!("{}={}", k, strings::TrimSpace(v));
                    query_list.push(query);
...
impl Requester for Request {
    ...
    fn set_biz_content(&mut self, b: &impl BizContenter) -> &mut Self {
        if b.is_biz_content() {
            self.biz_content = Some(get_biz_content_str(b));
        }
        self.borrow_mut()
    }
    ...
    fn set_param(&mut self, b: &impl BizContenter) -> &mut Self {
        if !b.is_biz_content() {
            self.param = Some(get_param_map(b));
        }
        self.borrow_mut()
    }
